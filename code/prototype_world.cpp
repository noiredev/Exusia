...
inline bool32 IsCanonical(real32 ChunkDim, real32 TileRel) {
    // TODO: Fix floating point math to be exact
...
inline void RecanonicalizeCoord(real32 ChunkDim, int32 *Tile, real32 *TileRel) {
    ...
    // TODO: Bounds checking to prevent wrapping
...
inline void ChangeEntityLocationRaw(memory_arena *Arena, world *World, uint32 LowEntityIndex,
                                 world_chunk *OldChunk, world_position *NewP) {
    // TODO: If this moves an entity into the camerma bounds, should it autoamtically go into the high set

    Assert(!NewP || IsValid(*NewP));

    if(OldChunk && NewP && AreInSameChunk(World, OldChunk, NewP)) {
        // NOTE: Leave entity where it is
    } else {
        if(OldChunk) {
            // NOTE: Pull the entity out of it's old entity block
            world_chunk *Chunk = OldChunk;
...
internal void ChangeEntityLocation(memory_arena *Arena, world *World,
                                   ...
                                   world_chunk *OldChunk, world_position NewPInit) {
    world_position *NewP = 0;
    if(IsValid(NewPInit)) {
        NewP = &NewPInit;
    }

    ChangeEntityLocationRaw(Arena, World, LowEntityIndex, OldChunk, NewP);

    if(NewP) {
        LowEntity->P = NewP->Offset_;
