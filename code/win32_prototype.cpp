...
internal void Win32GetEXEFileName(win32_state *State) {
    // NOTE: MAX_PATH is not recommended for shipping code
...
internal void Win32GetInputFileLocation(win32_state *State, bool32 InputStream, int SlotIndex, int DestCount, char *Dest) {
    char Temp[64];
    wsprintf(Temp, "loop_edit_%d_%s.gmi", SlotIndex, InputStream ? "input" : "state");
...
int CALLBACK WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CommandLine, int ShowCode) {
    ...
    Win32BuildEXEPathFileName(&Win32State, "prototype.dll", sizeof(SourceGameCodeDLLFullPath), SourceGameCodeDLLFullPath);

    char TempGameCodeDLLFullPath[WIN32_STATE_FILE_NAME_COUNT];
    Win32BuildEXEPathFileName(&Win32State, "prototype_temp.dll", sizeof(TempGameCodeDLLFullPath), TempGameCodeDLLFullPath);
...
//    WindowClass.hIcon;
    WindowClass.lpszClassName = "PrototypeWindowClass";
    ...
    if(RegisterClassA(&WindowClass)) {
        ...
        if(Window) {
            ...
            // TODO: Transient storage needs to be broken up
